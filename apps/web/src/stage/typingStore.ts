 ...
 * different attribution label (`wordTestLabel`).
 */
export type ActiveTest =
  | { kind: 'passage'; passage: Passage }
  | { kind: 'words'; text: string; count: number; drill?: boolean };
 ...
 * run, else "words · 50" (Batch C §2.3 will extend this with a punctuation/
 * numbers suffix once those toggles land).
 */
export function wordTestLabel(test: Extract<ActiveTest, { kind: 'words' }>): string {
  return `${test.drill === true ? 'drill' : 'words'} · ${String(test.count)}`;
}
